...
export default function Registration({ onNotify }) {
  ...
  const validate = () => {
    ...
    if (!form.role)                                   e.role            = 'Pumili ng iyong papel.'
    if (!form.paaralan.trim())                        e.paaralan        = 'Kailangan ang pangalan ng paaralan.'
    if (!form.kurso.trim())                           e.kurso           = 'Kailangan ang kurso o antas.'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'users', user.uid), {
        ...
        email:     form.email,
        role:      'user',           // system role — all new registrations default to 'user'
        papel:     form.role,        // user-selected papel (guro, manunulat, etc.)
      ...
      <div className={`auth-card${shake ? ' auth-card--shake' : ''}`}>
        ...
        <div className="auth-right">
          ...
          <form className="auth-form" onSubmit={handleSubmit} noValidate>
            ...
            {/* Papel / Role Dropdown */}
            <div className="auth-field">
              <label className="auth-label" htmlFor="rg-role">Papel / Tungkulin</label>
              <div className="auth-input-wrap">
                <RoleIcon />
                <select id="rg-role" className="auth-input auth-select"
                  value={form.role} onChange={handleChange('role')}
                  aria-invalid={!!errors.role}>
                  <option value="">— Pumili ng papel —</option>
                  <option value="guro">Guro</option>
                  <option value="manunulat">Manunulat</option>
                  <option value="estudyante">Estudyante</option>
                  <option value="tagasuri">Tagasuri</option>
                </select>
                <ChevronIcon />
              </div>
              {errors.role && <span className="auth-error-msg" role="alert">{errors.role}</span>}
            </div>

            {/* Paaralan & Kurso — side by side on wider screens */}
            <div className="auth-row">
              <div className="auth-field">
                <label className="auth-label" htmlFor="rg-paaralan">Paaralan</label>
                <div className="auth-input-wrap">
                  <SchoolIcon />
                  <input id="rg-paaralan" type="text" className="auth-input"
                    placeholder="Pangalan ng paaralan" value={form.paaralan} onChange={handleChange('paaralan')}
                    autoComplete="organization" aria-invalid={!!errors.paaralan} />
                </div>
                {errors.paaralan && <span className="auth-error-msg" role="alert">{errors.paaralan}</span>}
              </div>
              <div className="auth-field">
                <label className="auth-label" htmlFor="rg-kurso">Kurso</label>
                <div className="auth-input-wrap">
                  <KursoIcon />
                  <input id="rg-kurso" type="text" className="auth-input"
                    placeholder="Kurso o antas" value={form.kurso} onChange={handleChange('kurso')}
                    autoComplete="off" aria-invalid={!!errors.kurso} />
                </div>
                {errors.kurso && <span className="auth-error-msg" role="alert">{errors.kurso}</span>}
              </div>
            </div>
...
function RoleIcon() {
  return <svg className="auth-input-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
}
function SchoolIcon() {
  return <svg className="auth-input-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
}
function KursoIcon() {
  return <svg className="auth-input-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>
}
function ChevronIcon() {
  return <svg className="auth-select-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="6 9 12 15 18 9"/></svg>
}
